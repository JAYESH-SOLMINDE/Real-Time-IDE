...
export function useVoice(socket: Socket | null, roomId: string, username: string) {
  ...
  // ── Get combined stream (audio + optional video) ──
  const getCombinedStream = (audio: MediaStream, video?: MediaStream): MediaStream => {
    const tracks = [...audio.getAudioTracks()];
    if (video) tracks.push(...video.getVideoTracks());
    return new MediaStream(tracks);
  };

  // ── Create / rebuild peer connection ──
  const createPeer = useCallback((targetSocketId: string, isInitiator: boolean) => {
    if (!socket || !localStream.current) return null;

    // Close existing peer if any
    peers.current.get(targetSocketId)?.close();

    const peer = new RTCPeerConnection(ICE_SERVERS);
    const combined = getCombinedStream(localStream.current, localVideo.current || undefined);

    combined.getTracks().forEach(track => {
      peer.addTrack(track, combined);
    });

    peer.onicecandidate = (e) => {
      if (e.candidate) socket.emit('voice-ice', { to: targetSocketId, candidate: e.candidate });
    };
    ...
    peer.ontrack = (e) => {
      ...
      // Update remote user's stream for video rendering
  ...
  const toggleVideo = useCallback(async () => {
    ...
    if (videoEnabled) {
      ...
      localVideo.current = null;
      setVideoEnabled(false);
      socket?.emit('voice-video', { roomId, videoEnabled: false });

      // Rebuild peers without video
      peers.current.forEach((_, sid) => createPeer(sid, true));
    } else {
      // Turn ON video
      try {
        ...
        localVideo.current = videoStream;
        setVideoEnabled(true);
        socket?.emit('voice-video', { roomId, videoEnabled: true });

        // Rebuild peers with video
        peers.current.forEach((_, sid) => createPeer(sid, true));
      } catch {
        setError('Camera access denied.');
      }
    }
  }, [inVoice, videoEnabled, socket, roomId, createPeer]);
  ...
  useEffect(() => {
    ...
    socket.on('voice-join', ({ socketId, username: name }: { socketId: string; username: string }) => {
      setVoiceUsers(prev => [
        ...
        { socketId, username: name, muted: false, speaking: false, videoEnabled: false },
      ]);
    ...
    socket.on('voice-mute', ({ socketId, muted: isMuted }: { socketId: string; muted: boolean }) => {
      setVoiceUsers(prev => prev.map(u =>
        u.socketId === socketId ? { ...u, muted: isMuted } : u
      ));
    });

    socket.on('voice-speaking', ({ socketId, speaking: isSpeaking }: { socketId: string; speaking: boolean }) => {
      setVoiceUsers(prev => prev.map(u =>
        u.socketId === socketId ? { ...u, speaking: isSpeaking } : u
      ));
    });

    socket.on('voice-video', ({ socketId, videoEnabled: isVideo }: { socketId: string; videoEnabled: boolean }) => {
      setVoiceUsers(prev => prev.map(u =>
        u.socketId === socketId ? { ...u, videoEnabled: isVideo } : u
      ));
    });
